...
const NotesProvider = ({children}) => {
    ...
    const updateNote = async(_id, note) => {
        try {
            ...
            if(response.status === 201) {
                authDispatch({type: "UPDATE_NOTE", payload:{toastMessage: "Note Updated", data: response.data.notes}})
            }
        }catch(error) {
            console.log(error)
        }
    ...
    return(
        <NotesContext.Provider value={{ getNotes, addNote, deleteNote, updateNote, archiveNote }}>
            {children}
